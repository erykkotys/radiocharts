...
def test_0333_song_airplay_can_filter_specific_stations():
    ...
    assert 'cached_airplay_station_coverage(AIR_REV, tuple(sorted(song_station_ids)), song_air_start.isoformat(), song_air_end.isoformat())' in APP
...
def test_0333_song_station_filter_updates_grid_identity_and_keeps_global_date_bounds():
    ...
    assert 'song_cov = cached_airplay_coverage(AIR_REV, tuple(sorted(all_song_station_ids)))' in APP
